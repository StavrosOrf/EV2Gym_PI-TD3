...
class GeneralPowerDataManager:
    ...
    def __init__(self, datapath: str) -> None:
        ...
        print(
            f"Data scale: from {min_date.strftime('%Y-%m-%d')} to {max_date.strftime('%Y-%m-%d')}")
        ...
        self.active_power_cols = [
            col for col in self.df.columns if re.fullmatch(r'active_power(_\w+)?', col)]
        self.reactive_power_cols = [
            col for col in self.df.columns if re.fullmatch(r'reactive_power(_\w+)?', col)]
        self.renewable_active_power_cols = [col for col in self.df.columns if re.fullmatch(
            r'renewable_active_power(_\w+)?', col)]
        self.renewable_reactive_power_cols = [col for col in self.df.columns if re.fullmatch(
            r'renewable_reactive_power(_\w+)?', col)]
        self.price_col = [
            col for col in self.df.columns if re.fullmatch(r'price(_\w+)?', col)]
        ...
        print(
            f"Price columns: {self.price_col} (Indices: {[self.df.columns.get_loc(col) for col in self.price_col]})")
        ...
        self.reactive_power_max = self.df[self.reactive_power_cols].max(
        ).max() if self.reactive_power_cols else None
        self.reactive_power_min = self.df[self.reactive_power_cols].min(
        ).min() if self.reactive_power_cols else None
        ...
        self.price_min = self.df[self.price_col].min(
        ).values[0] if self.price_col else None
        self.price_max = self.df[self.price_col].max(
        ).values[0] if self.price_col else None
    ...
    def _check_for_nan(self) -> None:
        ...
        if self.df.isnull().sum().sum() > 0:
            raise ValueError(
                "Data still contains NaN values after preprocessing")

    def select_timeslot_data(self,
                             year: int,
                             month: int,
                             day: int,
                             hour: int,
        ...
        dt = pd.Timestamp(year=year, month=month, day=day, hour=hour, minute=minute, second=0, tz='UTC') + pd.Timedelta(
    ...
    def select_day_data(self, year: int, month: int, day: int) -> np.ndarray:
        ...
        """
        start_dt = pd.Timestamp(year=year, month=month,
                                day=day, hour=0, minute=0, second=0, tz='UTC')
    ...
    def list_dates(self) -> List[Tuple[int, int, int]]:
        ...
        year_month_day = [(int(date[:4]), int(date[5:7]),
                           int(date[8:10])) for date in dates]
    ...
    def split_data_set(self):
        ...
        for date in all_dates:
            ...
            if month != current_month or year != current_year:
                ...
                monthly_dates.sort()
                # First three weeks for training
