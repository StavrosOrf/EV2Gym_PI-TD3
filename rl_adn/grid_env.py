...
class PowerNetEnv():
    ...
    def reset(self, date) -> np.ndarray:
        ...
        date
        self.current_step = 0
        self.augmented_data = self.augmentor.sample_data(n_buses=self.node_num,
        ...
        plt.figure()
        for i in range(1):
            plt.step(self.augmented_data.index,
                     self.augmented_data.iloc[:, i], label=f'node_{i}')
        plt.show()
        exit()
    ...
    def _get_obs(self):
        ...
        one_slot_data = self.augmented_data[self.current_step]
        print(f'one_slot_data: {one_slot_data}')
        if self.algorithm == "Laurent":
