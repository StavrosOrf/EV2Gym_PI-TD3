...
def train_sac(env, state, key, step_fn, train_steps=1000, buffer_size=10000, batch_size=64, gamma=0.99, tau=0.005, alpha=0.2, eval_interval=200):
    ...
    key, subkey = jr.split(key)
    critic1_params = critic1.init(subkey, jnp.zeros(
        (1, obs_size)), jnp.zeros((1, act_size)))
    key, subkey = jr.split(key)
    critic2_params = critic2.init(subkey, jnp.zeros(
        (1, obs_size)), jnp.zeros((1, act_size)))
    ...
    for _ in range(buffer_size):
        ...
        buffer.add((state.obs[0], action[0], next_state.reward[0],
    ...
    def sac_update(a_p, c1_p, c2_p, c1_t, c2_t, a_os, c1_os, c2_os, s_b, a_b, r_b, n_b, d_b):
        def critic_loss_fn(c1p, c2p):
            ...
            log_prob = jnp.sum(-0.5 * (((z2)**2 + 2 *
                               next_log_std + jnp.log(2*jnp.pi))), axis=-1)
            ...
            return loss1 + loss2
        (c_loss), (c1_grads, c2_grads) = jax.value_and_grad(
            critic_loss_fn, argnums=(0, 1))(c1_p, c2_p)
        ...
        def actor_loss_fn(ap):
            ...
            logp = jnp.sum(-0.5 * (((z3)**2 + 2*log_std +
        ...
        c1_t = jax.tree_util.tree_map(
            lambda t, s: t * (1 - tau) + s * tau, c1_t, c1_p)
        c2_t = jax.tree_util.tree_map(
            lambda t, s: t * (1 - tau) + s * tau, c2_t, c2_p)
    ...
    for i in range(train_steps):
        ...
        if i % eval_interval == 0:
            ...
            evaluate_policy(env, SACActor, actor_params)
    return actor_params
